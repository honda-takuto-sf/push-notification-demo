 ...
 */
export const sendScheduledNotification = async (token: string, title: string, body: string, delay: number, icon_url: string, image: string): Promise<object> => {
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/send-scheduled-notification/`, {
            ...
            body: JSON.stringify({ token, title, body, delay, icon_url, image}),
            credentials: 'include'
        });
